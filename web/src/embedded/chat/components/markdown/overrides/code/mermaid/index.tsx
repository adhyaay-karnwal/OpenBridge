...
import { useDeferredRender } from '@/embedded/chat/hooks/use-defered-render';
...
export const Mermaid = ({
  ...
  useEffect(() => {
    ...
    renderChart();
  }, [chart, config, lastValidSvg, retryCount, shouldRender, svgContent]);
  ...
  if (error && !svgContent && !lastValidSvg) {
    ...
    return (
      <div className={cn('p-4', className)} ref={containerRef}>
        <p className="font-mono text-apple-red text-sm">
          Mermaid Error: {error}
        </p>
        <details className="mt-2">
          <summary className="cursor-pointer text-apple-red text-xs">
            Show Code
          </summary>
          <pre className="mt-2 overflow-x-auto rounded-xl bg-black/10 dark:bg-white/10 p-2 text-apple-red text-xs">
            {chart}
          </pre>
        </details>
      </div>
