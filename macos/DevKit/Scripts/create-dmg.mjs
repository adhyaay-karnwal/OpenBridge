...
/**
 * Create styled DMG using appdmg (no AppleScript required)
 *
...
function usage() {
  ...
  console.log('');
  console.log('Creates a styled DMG with custom background and icon positions.');
...
function main() {
  ...
  // Determine background image based on scale
  let backgroundImage = join(INSTALLER_RESOURCES, 'install-bg.jpg');
  const scale = config.scale || 1;

  if (scale === 3 && existsSync(join(INSTALLER_RESOURCES, 'install-bg@3x.jpg'))) {
    backgroundImage = join(INSTALLER_RESOURCES, 'install-bg@3x.jpg');
    console.log('[i] using @3x background image');
  } else if (scale === 2 && existsSync(join(INSTALLER_RESOURCES, 'install-bg@2x.jpg'))) {
    backgroundImage = join(INSTALLER_RESOURCES, 'install-bg@2x.jpg');
    console.log('[i] using @2x background image');
  } else {
    console.log('[i] using @1x background image');
  }
  ...
  console.log(`[i] background: ${backgroundImage}`);
