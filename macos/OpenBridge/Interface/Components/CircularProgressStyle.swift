...
#Preview {
    VStack(spacing: 20) {
        // Indeterminate mode - Ring style (default)
        ProgressView()
            .progressViewStyle(CircularProgressStyle())

        ProgressView()
            .progressViewStyle(CircularProgressStyle(color: .blue, size: 24))

        // Indeterminate mode - Rays style
        ProgressView()
            .progressViewStyle(CircularProgressStyle(indeterminateStyle: .rays))

        ProgressView()
            .progressViewStyle(CircularProgressStyle(color: .blue, size: 24, indeterminateStyle: .rays))
        ...
        ProgressView(value: 0.8)
            .progressViewStyle(CircularProgressStyle(color: .green, lineWidth: 3, size: 32))
    }
    .padding()
